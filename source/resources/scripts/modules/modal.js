...
function modalController($scope, biography, $sce, states, $http, Popeye) {
    ...
    $scope.register = function register(registrant) {
        ...
        .catch(function error(response) {
            ...
            angular.forEach(response.data, function(errors, fieldName) {
                $scope.contestRegistrationForm[fieldName].$setValidity('server', false);
            });
        })
...
.directive('serverValidates', function serverValidatesDirective() {
    return {
        restrict: 'A',
        require: '?ngModel',
        link: function serverValidatesDirectiveLink(scope, elem, attrs, ctrl) {
            elem.on('change', function serverValidatesDirectiveChanged() {
